    ...
    function handleData(data) {
      ...
      for (var i = 2; i < data.length; ++i) {
        ...
        if(i % 2) {
          ...
          for (var attr in data[i].nodes) {
			  if(typeof dataNodes.nodes[attr]!=='undefined') {
				  if(moment.utc(dataNodes.nodes[attr].lastseen).local().isAfter(moment.utc(data[i].nodes[attr].lastseen).local()))
				  continue
			  }
			  dataNodes.nodes[attr] = data[i].nodes[attr]
			}
      }
